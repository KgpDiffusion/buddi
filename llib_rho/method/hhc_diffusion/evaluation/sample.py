...
from llib_rho.visualization.diffusion_eval import save_gif, render_images
from llib_rho.visualization.scripts.tools import build_renderer, render_360_views

from llib_rho.method.hhc_diffusion.evaluation.utils import *
...
def parse_args():
    ...
    parser.add_argument("--eval-dataset-name", type=str, default="behave")
...
def save_result(OUTPUT_FOLDER, x_ts, x_starts):
    ...
    with open(f"{OUTPUT_FOLDER}/x_starts.pkl", "wb") as f:
        pickle.dump(x_starts, f)

    with open(f"{OUTPUT_FOLDER}/x_ts.pkl", "wb") as f:
        pickle.dump(x_ts, f)
...
def save_batch_gifs(
    output_folder, renderer, verts, faces, verts_obj, faces_obj, suffix="", num_poses=30, fps=6, **kwargs
):
    """
    :param output_folder
    ...
    for i in range(len(verts)):
        frames = render_360_views(renderer, verts[i], faces, verts_obj[i], faces_obj, num_poses, **kwargs)
        frames = np.array(frames)
        imageio.mimwrite(f"{output_folder}/{i:05d}{suffix}.gif", frames[:, :, :, :3], fps=fps, loop=10)
...
def main(cfg, cmd_args):
    ...
    if INPAINT:
        ...
        x_ts, x_starts, obj_face_tensor = output

    final_vertices = x_starts["final"]["vertices"]
    print("final verts shape", final_vertices.shape)
    final_obj_vertices = x_starts["final"]["vertices_obj"]
    print("final verts object shape", final_obj_vertices.shape)
    ...
    if cmd_args.save_vis:
        ...
        # obj_faces = diffusion_module.obj_faces.to("cpu")
        obj_faces = obj_face_tensor.to("cpu")
        save_batch_gifs(VIS_FOLDER, renderer, final_vertices, faces, final_obj_vertices, obj_faces, suffix="_gen", show_ground=cmd_args.render_floor)
