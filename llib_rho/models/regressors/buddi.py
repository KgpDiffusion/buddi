...
class BUDDI(nn.Module):
    ...
    def add_param_embedding(self, x, param_name):
        if self.use_param_embedding:
    ...
    def forward(self, x, timesteps, guidance={}, return_latent_vec=False):
        ...
        for k, x_param in x.items():
            ...
            xemb = self.add_param_embedding(xemb, k)
            emb_x.append(xemb) # xemb shape - (B, 1, 152) # tentative
        ...
        if self.use_cross_attention:
            ...
            xx = torch.cat((emb_x + emb_timesteps + emb_guidance), dim=1) # Shape- (B, 6+1+6, 152)
